...
bpnn_func <- function(X_train, y_train, X_test, y_test, k){
          ...
          grdnt_dscnt_exp <- function(n_prmtr, w_prmtr, e_in, yita, y_1, y_2, data){
                    n <- n_prmtr; p <- ncol(data)
                    w_iter <- rbind(w_prmtr[j,], matrix(data = rep(0,n*p), nrow = n, ncol = p, byrow = T))
                    for(i in 2:nrow(w_iter)){
                              w_iter[i,] <- w_iter[i-1,] - yita*vctr_idnt(t(-(y_1-y_2)*exp(data%*%w_iter[i-1,]))%*%data)
                              e_in[i] <- sum((y_1-exp(data%*%w_iter[i,]))^2)/2
                              if(round(e_in[i],7) == round(e_in[i-1],7)){break}
                    }
                    res <- list(w_iter = w_iter, e_in = e_in)
                    return(res)
          ...
          for(j in 1:k){
                    ...
                    # 4.w6_update
                    res6 <- grdnt_dscnt_exp(n_prmtr = 50000, w_prmtr = w6, e_in = 1000, yita = 0.0001, y_1 = y_train, y_2 = y_hat, data = layer)
                    plot(res6$e_in[-1]); min(res6$e_in[-1]); which.min(res6$e_in[-1])+1
                    w6 <- rbind(w6, res6$w_iter[which.min(res6$e_in[-1])+1,])
                    
                    # 5.hidden_update
                    d_hddn <- (-(y_train-y_hat)*exp(layer%*%w6[j+1,]))%*%w6[j+1,]
                    d_hddn1 <- d_hddn[,2]
                    d_hddn2 <- d_hddn[,3]
                    d_hddn3 <- d_hddn[,4]
                    d_hddn4 <- d_hddn[,5]
